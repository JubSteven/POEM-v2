...
class OakInk(HDataset):
    ...
    # for mv dataset
    def get_cam_serial(self, idx):
        info = self.info_list[idx]
        return self.framedata_color_name[info[3]]
    ...
    def _getitem_3d(self, idx):
        ...
        # Flip 2d if needed (TODO: use cv2.affineTrans)
        if flip_hand:
        ...
        cam_serial = self.get_cam_serial(idx)
...
class OakInkMultiView(torch.utils.data.Dataset):
    ...
    def __getitem__(self, sample_idx):
        ...
        if self.master_system == "as_constant_camera":
            ...
            master_id = 0
            T_master_2_new_master = sample["cam_extr"][master_id].copy()
            master_joints_3d = sample["target_joints_3d_no_rot"][master_id]
            master_verts_3d = sample["target_verts_3d_no_rot"][master_id]
        elif self.master_system == "as_first_camera":
            master_id = 0  # idx_list[0]
            T_master_2_new_master = sample["cam_extr"][0].copy()
            master_joints_3d = sample["target_joints_3d_no_rot"][0]
            master_verts_3d = sample["target_verts_3d_no_rot"][0]
        else:
        ...
        if self.random_n_views:
            ...
            for key, value in sample.items():
                if not isinstance(value, int) and len(value) == self.n_views:  # process info with len == N_VIEW
