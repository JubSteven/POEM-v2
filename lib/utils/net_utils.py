...
def build_scheduler(optimizer: Optimizer, cfg, steps=None):
    ...
    if isinstance(lr_decay_step, list) and scheduler == "StepLR":
        ...
        assert steps != None, "steps should be provided for CosineLR"
        tar_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max = steps,
            eta_min = cfg.LR_MIN,
        )    
...
def load_weights(module: nn.Module, pretrained=None, strict=True):
    if pretrained == "" or pretrained is None:
        ...
        logger.info(f"=> Loading {type(module).__name__} pretrained model from: {pretrained}")
