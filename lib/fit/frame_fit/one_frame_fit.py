...
from manotorch.utils.quatutils import (
    _normalize_quaternion as normalize_quaternion,
    _quaternion_to_angle_axis as quaternion_to_angle_axis,
)
...
class OneFrameFit:
    ...
    def setup(
        ...
        self._keypoint_arr = (
            const_keypoint_arr[..., 0:2].clone().detach().float().to(self.device)
        )  # [NPERSP, 21, 2]
        self._keypoint_weight_arr = (
            const_keypoint_arr[..., 2].clone().detach().float().to(self.device)
        )  # [NPERSP, 21]
        self._cam_intr_arr = (
            const_cam_intr_arr.clone().detach().float().to(self.device)
        )  # [NPERSP, 3, 3]
        self._cam_extr_arr = (
            const_cam_extr_arr.clone().detach().float().to(self.device)
        )  # [NPERSP, 4, 4]
        self._n_persp = int(self._keypoint_arr.shape[0])

        self._hand_pose = (
            init_hand_pose.float().detach().clone().to(self.device)
        )  # (1, 64)
        self._hand_shape = (
            init_hand_shape.float().detach().clone().to(self.device)
        )  # (1, 10)
        self._hand_tsl = (
            init_hand_tsl.float().detach().clone().to(self.device)
        )  # (1, 3)
        ...
        if fit_hand_shape:
            ...
            param_to_optimize.append(
                {"params": [self._hand_shape], "lr": 0.1 * self.lr}
            )
        if fit_hand_tsl:
    ...
    def step(self):
        ...
        vec_pose_normalized = normalize_quaternion(
            vec_pose.reshape((-1, 16, 4))
        )  # (B=1, 16, 4)
        n_var_pose = 15
        init_pose_zero = np.array([[1.0, 0.0, 0.0, 0.0]] * n_var_pose)[
            None, ...
        ].astype(
            np.float32
        )  # (B=1, 15, 4)
        init_pose_zero = torch.tensor(
            init_pose_zero, dtype=torch.float, device=self.device
        ).expand((bsize, -1, -1))
        pose_reg_loss = HandLoss.pose_reg_loss(
            vec_pose_normalized[:, 1:, :], init_pose_zero
        )
        init_shape_zero = torch.zeros(
            (1, 10), dtype=torch.float, device=self.device
        ).expand(
            (bsize, -1)
        )  # (B=1, 10)
        ...
        b_axis, u_axis, l_axis = self.axis_layer(
            rebuild_joints, rebuild_transforms
        )  # (B, 15, 3)
        ...
        hand_anatomical_loss = (
            1.0 * pose_quat_norm_loss
            + 0.0 * pose_reg_loss
            + 0.1 * shape_reg_loss
        ...
        multi_cam_rebuild_joints = batch_cam_extr_transf(
            self._cam_extr_arr[None, ...].expand(
                bsize, -1, -1, -1
            ),  # (B=1, NPERSP, 4, 4)
            rebuild_joints.unsqueeze(1).expand(
                -1, self._n_persp, -1, -1
            ),  # (B=1, NPERSP, 21, 3)
        )  # [B, NPERSP, 21, 3]
        multi_cam_projected_joints = batch_cam_intr_projection(
            self._cam_intr_arr[None, ...].expand(
                bsize, -1, -1, -1
            ),  # (B, NPERSP, 3, 3)
            multi_cam_rebuild_joints,  # [B, NPERSP, 21, 3]
        )  # [B, NPERSP, 21, 2]

        multi_cam_projected_offset = (
            multi_cam_projected_joints - self._keypoint_arr
        ) / self._image_scale
        multi_cam_projected_distance = torch.sum(
            torch.pow(multi_cam_projected_offset, 2), dim=3
        )  # [B, NPERSP, 21]

        multi_cam_projected_distance = (
            multi_cam_projected_distance * self._keypoint_weight_arr
        )
        reprojection_loss = torch.mean(multi_cam_projected_distance)
        # endregion <<<<<<

        loss = (
            self.lambda_reprojection_loss * reprojection_loss
        ...
        if self.optimizing:
            ...
            if self.grad_clip is not None:
                torch.nn.utils.clip_grad_norm_(
                    [self._hand_pose, self._hand_shape, self._hand_tsl], self.grad_clip
                )
            self.optimizer.step()
            self.scheduler.step(loss)

        self.curr_iter += 1
        return reprojection_loss.item()
