...
from manotorch.utils.quatutils import (
    _quaternion_norm_squared as quaternion_norm_squared,
    _quaternion_inv as quaternion_inv,
    _quaternion_mul as quaternion_mul,
)
...
class HandLoss:
    ...
    def joint_b_axis_loss(b_axis, axis, angle_mask):
        ...
        b_thumb_soft_idx = [12]
        b_restrict_idx = [
            i for i in range(15) if i not in b_soft_idx and i not in b_thumb_soft_idx
        ]
        ...
        soft_loss = torch.relu(
            torch.abs(b_axis_cos[:, b_soft_idx]) - np.cos(np.pi / 2 - np.pi / 36)
        )  # [-5, 5]
        thumb_soft_loss = torch.relu(
            torch.abs(b_axis_cos[:, b_thumb_soft_idx]) - np.cos(np.pi / 2 - np.pi / 9)
        )  # [-60, 60]

        res = (
            torch.mean(torch.pow(restrict_cos * angle_mask[:, b_restrict_idx], 2))
            + torch.mean(torch.pow(soft_loss * angle_mask[:, b_soft_idx], 2))
            + torch.mean(
                torch.pow(thumb_soft_loss * angle_mask[:, b_thumb_soft_idx], 2)
            )
        )
        return res
    ...
    def joint_u_axis_loss(u_axis, axis, angle_mask):
        ...
        u_thumb_soft_idx = [12]
        u_restric_idx = [
            i for i in range(15) if i not in u_soft_idx and i not in u_thumb_soft_idx
        ]

        u_axis_cos = torch.einsum("bki,bki->bk", u_axis, axis)
        restrict_cos = u_axis_cos[:, u_restric_idx]
        soft_loss = torch.relu(
            torch.abs(u_axis_cos[:, u_soft_idx]) - np.cos(np.pi / 2 - np.pi / 6)
        )  # [-10, 10]
        thumb_soft_loss = torch.relu(
        ...
        res = (
            torch.mean(torch.pow(restrict_cos * angle_mask[:, u_restric_idx], 2))
            + torch.mean(torch.pow(soft_loss * angle_mask[:, u_soft_idx], 2))
            + torch.mean(
                torch.pow(thumb_soft_loss * angle_mask[:, u_thumb_soft_idx], 2)
            )
        )
        return res
    ...
    def joint_l_limit_loss(l_axis, axis, angle_mask):
        ...
        l_thumb_soft_idx = [12]
        l_restrict_idx = [
            i for i in range(15) if i not in l_soft_idx and i not in l_thumb_soft_idx
        ]

        l_axis_cos = torch.einsum("bki,bki->bk", l_axis, axis)
        restrict_cos = l_axis_cos[:, l_restrict_idx]
        soft_loss = torch.relu(
            -l_axis_cos[:, l_soft_idx] + 1 - np.cos(np.pi / 2 - np.pi / 9)
        )  # [-20, 20]
        thumb_soft_loss = torch.relu(
            -l_axis_cos[:, l_thumb_soft_idx] + 1 - np.cos(np.pi / 2 - np.pi / 3)
        )

        res = (
            torch.mean(torch.pow((restrict_cos - 1) * angle_mask[:, l_restrict_idx], 2))
            + torch.mean(torch.pow(soft_loss * angle_mask[:, l_soft_idx], 2))
            + torch.mean(
                torch.pow(thumb_soft_loss * angle_mask[:, l_thumb_soft_idx], 2)
            )
        )
        return res
    ...
    def rotation_angle_loss(angle, limit_angle=np.pi / 2, eps=1e-10):
        ...
        angle_new[nonzero_mask] = angle[
            nonzero_mask
        ]  # if angle is too small, pick them out of backward graph
        angle_over_limit = torch.relu(
            angle_new - limit_angle
        )  # < pi/2, 0; > pi/2, linear | Tensor[16, ]
